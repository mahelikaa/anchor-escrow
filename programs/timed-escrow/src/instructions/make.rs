...
pub struct Make<'info> {
    ...
    #[account(
        init,
        payer = maker,
        space = 8 + TimedEscrow::INIT_SPACE,
        seeds = [TIMED_ESCROW_SEED, maker.key().as_ref(), seed.to_le_bytes().as_ref()],
        bump
    )]
...
pub fn handle_make(
    ...
    require!(deposit > 0, TimedEscrowError::InvalidAmount);
    require!(receive > 0, TimedEscrowError::InvalidAmount);
    require_keys_neq!(
        ctx.accounts.mint_a.key(),
        ctx.accounts.mint_b.key(),
        TimedEscrowError::IdenticalMints
    );

    // An escrow that is already expired could never be taken, only refunded.
    let now = Clock::get()?.unix_timestamp;
    require!(expires_at > now, TimedEscrowError::InvalidDeadline);

    ctx.accounts.escrow.set_inner(TimedEscrow {
        seed,
