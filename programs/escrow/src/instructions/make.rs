...
pub struct Make<'info> {
    ...
    /// Custodies the deposit. Its authority is the escrow PDA, so only this
    /// program can move the tokens back out.
    #[account(
        init,
        payer = maker,
        associated_token::mint = mint_a,
...
pub fn handle_make(ctx: Context<Make>, seed: u64, deposit: u64, receive: u64) -> Result<()> {
    ...
    // The maker signs for their own tokens, so no PDA signature is needed here.
